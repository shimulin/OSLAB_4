...
static int osfs_create(struct mnt_idmap *idmap, struct inode *dir, struct dentry *dentry, umode_t mode, bool excl) {
    ...
    struct osfs_inode *parent_inode = dir->i_private; //透過 dir->i_private 取得父目錄的 osfs_inode

    // Step2: Validate the file name length
    if (dentry->d_name.len > MAX_FILENAME_LEN) { //若檔案名稱超過 MAX_FILENAME_LEN，回傳 -ENAMETOOLONG
        return -ENAMETOOLONG;
    }

    // Step3: Allocate and initialize VFS & osfs inode
    struct inode *inode = osfs_new_inode(dir, mode); //使用 osfs_new_inode() 為新檔案分配 inode 並初始化。

    // Step4: Parent directory entry update for the new file
    osfs_add_dir_entry(dir, inode->i_ino, dentry->d_name.name, dentry->d_name.len); //呼叫 osfs_add_dir_entry()，在父目錄的資料區塊中新增一個條目。

    // Step 5: Update the parent directory's metadata
    parent_inode->__i_mtime = parent_inode->__i_atime = current_time(dir); //更新父目錄 inode 的 i_size 和時間戳（__i_mtime 與 __i_atime）
